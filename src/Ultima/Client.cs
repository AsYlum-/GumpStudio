    ...
    public static class Client
    {
        ...
        public static ProcessStream ProcessStream
        {
            get
            {
                ...
                {
                    m_ProcStream = Running ? new WindowProcessStream(Handle) : null;
                }
        ...
        /// </summary>
        public static List<string> Directories => m_Directories ?? (m_Directories = LoadDirectories());
        ...
        public static string GetFilePath(string file)
        {
            ...
            return dirs.Select(t => Path.Combine(t, file)).FirstOrDefault(File.Exists);
        }
        ...
        private static string GetExePath(string subName)
        {
            try
            {
                ...
                if (!(key.GetValue("ExePath") is string path) || path.Length == 0 || !Directory.Exists(path) && !File.Exists(path))
                {
        ...
        private static List<string> LoadDirectories()
        {
            ...
            foreach (var key in knownRegkeys)
            {
                var exePath = IntPtr.Size == 8
                    ? GetExePath(@"Wow6432Node\" + key)
                    : GetExePath(key);
            ...
            foreach (var directory in knownDirectories)
            {
                ...
                {
                    dirs.Add(Path.GetDirectoryName(directory));
                }
            }
        ...
        private static readonly string[] knownDirectories = new string[]
        {
